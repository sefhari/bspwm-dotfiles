

BASE_PACKAGES = [
    "tumbler", "ffmpegthumbnailer", "lsd", "alacritty", "bat", "brightnessctl", "calc",
    "automake", "dunst", "fakeroot", "feh", "firefox",
    "dpkg", "gcc", "gedit", "git", "gnu-netcat", "htop", "btop", "lxappearance",
    "mat2", "mpd", "mpv", "thunar", "ncmpcpp", "neofetch", "network-manager-applet", "nitrogen",
    "pamixer", "papirus-icon-theme", "pavucontrol", "polybar", "autoconf", "mpc",
    "ranger", "redshift", "reflector", "rofi", "rofi-calc", "calcurse",
    "rofi-emoji", "scrot", "sudo", "slop", "tree", "unrar", "zip", "unzip", "uthash", "xarchiver",
    "xfce4-power-manager", "xfce4-settings", "xorg-xbacklight", "zathura", "zathura-djvu", "zathura-pdf-mupdf",
    "cmake", "clang", "gzip", "imagemagick", "make", "openssh", "shellcheck",
    "usbutils", "picom", "alsa-plugins", "alsa-tools", "alsa-utils", "ffmpeg",
    "p7zip", "gparted", "sshfs", "xclip", "gpick", "wget", "ueberzug", "netctl", "light", "libreoffice",
    "breeze", "thunderbird", "copyq", "micro"
]

DEV_PACKAGES = [
    "timeshift", "pinta",
    "ghex", "python-pywal",
    "bleachbit", "gtkhash", "dconf-editor",
    "telegram-desktop", "youtube-dl", "code", "cowsay",
    "deluge-gtk", "flameshot", "obsidian", "cloc", "zsh"
]

AUR_PACKAGES = [
    "cava", "i3lock-color", "64gram-desktop-bin", "birdtray", "nekoray-bin", "all-repository-font"
]

GNOME_OFFICIAL_TOOLS = [
    "evince", "gnome-calculator", "gnome-disk-utility", "gucharmap",
    "gthumb", "gnome-clocks"
]
